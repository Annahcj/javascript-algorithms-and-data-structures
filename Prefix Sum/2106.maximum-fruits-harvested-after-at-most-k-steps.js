...
var maxTotalFruits = function(fruits, startPos, k) {
  ...
  for (let [idx, fruit] of fruits) pSum[idx] = fruit;
  for (let i = 1; i < max; i++) pSum[i] += pSum[i - 1];
  for (let right = Math.min(max - 1, startPos + k); right >= startPos; right--) {
  ...
  for (let left = Math.max(0, startPos - k); left <= startPos; left++) {
