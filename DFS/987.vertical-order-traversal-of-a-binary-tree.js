...
var verticalTraversal = function(root) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < res[i].length; j++) res[i][j] = res[i][j][1];
  }
  return res;
...
var verticalTraversal = function(root) {
  ...
  let res = [];
  for (let i = 0; i < nodes.length; i++) { // group together nodes that share the same column, and filter to only keep the node value.
