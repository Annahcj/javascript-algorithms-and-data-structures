...
var minCost = function(grid) {
  ...
  for (var i = 0; i < n; i++) dp[i] = Array(m).fill(Infinity);
  let cost = 0;
  let queue = [];
  dfs(0, 0, cost);
  ...
  while (queue.length) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      for (var j = 0; j < 4; j++) dfs(row + directions[j][0], col + directions[j][1], cost); // change the direction of the arrow with a cost of 1
    }
  }
  return dp[n - 1][m - 1];
